...
| `agent-loop.ts` | 1リクエスト分のループ。llama-server呼び出し→tool実行→再呼び出し | mcp-registry, tools |
| `server.ts` | HTTPで受けて agent-loop に渡し、結果を返す | agent-loop |

境界の確認:
- `tools.ts` は純粋な変換ロジックで、外部I/Oを持たない（単体テスト可能）。
...
- `agent-loop.ts`: llama-server と子MCPをモックして、(1) ツール呼び出し→実行→最終回答のループ、(2) 反復上限での打ち切り、(3) ツールエラーを結果に詰めて継続、を検証。
...
- 名前付きツールの強制実行（OpenAI `tool_choice`）による「〇〇を使って」の確実な遵守。
